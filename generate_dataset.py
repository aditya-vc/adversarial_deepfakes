...
fd = face_detector.FaceAlignmentDetector(lmd_weights_path='/home/meet_himanshu_dutta/dd-research/adversarial_deepfakes/face_toolbox_keras/models/detector/FAN/2DFAN-4_keras.h5', fd_weights_path='/home/meet_himanshu_dutta/dd-research/adversarial_deepfakes/face_toolbox_keras/models/detector/s3fd/s3fd_keras_weights.h5')
idet = IrisDetector()
model = FaceTranslationGANInferenceModel()

image_locations = glob.glob('pics/*')
image_locations = [fl for fl in image_locations if fl.endswith('.jpg') or fl.endswith('.png') or fl.endswith('.jpeg')]
...
#     try:
    ...
    plt.savefig('pics/abc/{}_{}.jpg'.format(fn_src_name, fn_tar_name))
#     except: 
#         print(fn_src, fn_tar)
...
#     img = plt.imread(fn_src)
#     plt.imshow(img)
#     fn_src_name = (fn_src.split("/")[-1]).split(".")[0]
#     plt.savefig('pics/abc/{}.jpg'.format(fn_src_name))